...
Fusabi (Functional Scripting for Rust) is an experimental Mini-F# dialect with an embeddable Rust bytecode VM, designed to replace Lua-style scripting in applications like terminal emulators (e.g., WezTerm). The goal is to provide F#-style developer ergonomics (records, DUs, pattern matching, pipelines) in a small, eager, expression-oriented language suitable for embedded scripting and configuration.
...
- **Skeletal Rust workspace**: `fusabi-frontend`, `fusabi-vm`, `fusabi-demo` crates
...
**Tasks**:
1. **Core AST** (`fusabi-frontend/src/ast.rs`):
...
**Tasks**:
1. **Value Representation** (`fusabi-vm/src/value.rs`):
   ```rust
   pub enum Value {
       Int(i64),
       Bool(bool),
...
2. **Bytecode** (`fusabi-vm/src/bytecode.rs`):
   ```rust
...
3. **VM Interpreter** (`fusabi-vm/src/vm.rs`):
...
**Tasks**:
1. **Bytecode Compilation** (`fusabi-frontend/src/compiler.rs`):
   - Compile AST to bytecode chunks
   - Constant pool management
   - Jump offset calculation

2. **Demo Host** (`fusabi-demo/src/main.rs`):
   - Load `.fsx` files
   - Compile to bytecode
   - Execute in VM
   - Display results
...
**Success Criteria**:
- Execute: `examples/arithmetic.fsx` with additions, multiplications
- Execute: `examples/conditional.fsx` with if/then/else
...
- **Target**: 3 real-world applications using Fusabi
...
4. Use `docs/claude-code-notes.md` for detailed prompts

### For Users (Post-v1.0)

1. Install: `cargo install fusabi`
