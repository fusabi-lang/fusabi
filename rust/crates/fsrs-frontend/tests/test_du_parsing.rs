...
use fsrs_frontend::ast::{DuTypeDef, Expr};
use fsrs_frontend::lexer::Lexer;
use fsrs_frontend::parser::Parser;

// Helper function to parse DU type definition
fn parse_du_typedef(input: &str) -> Result<DuTypeDef, fsrs_frontend::parser::ParseError> {
...
fn parse_expr(input: &str) -> Result<Expr, fsrs_frontend::parser::ParseError> {
    let mut lexer = Lexer::new(input);
    let tokens = lexer.tokenize().unwrap();
    let mut parser = Parser::new(tokens);
    parser.parse()
